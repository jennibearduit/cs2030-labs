...
class BusStop {
  ...
  /**
   * Return a Completable Future of set of bus services that serve 
   * this bus stop as a set.  Query the web server.
   * @return A Completable Future of set of BusService 
   * that serve this bus stop.
   */
  public CompletableFuture<Set<BusService>> getBusServices() {
    // Thread.sleep(200);
    // bus services that visit this stop

    return BusAPI.getBusServicesAt(stopId).thenApply(bs -> {
